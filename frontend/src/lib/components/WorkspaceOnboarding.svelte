...
<div class="flex min-h-[60vh] flex-col items-center justify-center p-6">
	<div class="w-full max-w-md space-y-6 rounded-2xl border border-border bg-surface p-8 shadow-lg">
		<div class="text-center">
			<div class="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-2xl border border-border bg-accent-soft text-accent">
			...
			<p class="mt-1 text-sm text-muted">Create your first workspace to get started with knowledge sources and chat.</p>
		</div>

		{#if errorMessage}
			<div class="rounded-xl border border-destructive-border bg-destructive-soft p-3 text-xs text-destructive-fg">
				{errorMessage}
			</div>
		{/if}
		...
		<form onsubmit={handleSubmit} class="space-y-4">
			<div>
				<label for="workspace-name" class="block text-xs font-semibold text-text mb-1">Workspace Name *</label>
				<input
					id="workspace-name"
					type="text"
					bind:value={name}
			...
			<div>
				<label for="workspace-slug" class="block text-xs font-semibold text-text mb-1">Custom Slug (Optional)</label>
				<input
					id="workspace-slug"
					type="text"
					bind:value={slug}
					...
					class="w-full rounded-xl border border-border bg-surface-raised px-4 py-2.5 text-sm text-text focus:border-accent focus:outline-none disabled:opacity-50"
				/>
				<p class="mt-1 text-[11px] text-muted">Leave empty to auto-generate a unique URL-friendly slug.</p>
			</div>
